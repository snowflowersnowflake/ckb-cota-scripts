...
const VERSION_INVALID: i8 = 6;
const WITNESS_TYPE_PARSE_ERROR: i8 = 8;
const SMT_PROOF_VERIFY_FAILED: i8 = 9;
const REGISTRY_CELL_SMT_ROOT_ERROR: i8 = 10;
const REGISTRY_DATA_INVALID: i8 = 12;
const REGISTRY_CELLS_COUNT_ERROR: i8 = 14;
...
fn generate_smt_data() -> ([u8; 32], [u8; 32], Vec<u8>) {
    ...
    let entries_builder = CotaNFTRegistryEntriesBuilder::default();
    let registry_vec_builder = RegistryVecBuilder::default();
    let merkel_proof_bytes = BytesBuilder::default()
        .extend(merkel_proof_vec.iter().map(|v| Byte::from(*v)))
        .build();

    let witness_data = entries_builder
        .registries(registry_vec_builder.set(registry_vec).build())
...
fn create_test_context(
    ...
    let registry_bin: Bytes = Loader::default().load_binary("registry-type");
    ...
    let registry_type_args = match registry_error {
        RegistryError::TypeArgsInvalid => Bytes::copy_from_slice(&ret[10..30]),
    ...
    let outputs_data: Vec<Bytes> = match (action, registry_error) {
        (Action::Update, RegistryError::RegistryCellSMTRootError) => {
            vec![Bytes::from(hex::decode("00").unwrap())]
        }
        ...
        (Action::Create, RegistryError::VersionInvalid) => {
            vec![Bytes::from(hex::decode("01").unwrap())]
        }
...
fn test_destroy_registry_cell_error() {
    ...
    assert_script_error(err, REGISTRY_CELLS_COUNT_ERROR);
}
...
#[test]
fn test_registry_version_error() {
    let (mut context, tx) = create_test_context(Action::Create, RegistryError::VersionInvalid);
